...
if __name__ == '__main__':
    ...
    all_sentences = np.load('all_sentences.npy')
    sentence_list = all_sentences.tolist()
    for sentence in sentence_list:
        if len(sentence) < 3 or len(sentence) > 25:
            sentence_list.remove(sentence)
    random.seed(0)
    sample_sentences = random.sample(sentence_list, 100)
    np.save('sample_sentences.npy', sample_sentences)
    '''
    for sentence in sample_sentences:
        for word_predict in ner.predict(sentence):
            predict.append(word_predict)
    print(predict)
    print(len(predict))
    ...
    with open('text.txt', 'r', encoding='UTF-8') as f:
        ...
        for i in range(int(len(text) / 10 )):
            for word_label in ner.predict(text[i * 10: (i + 1) * 10]):
                predict.append(word_label)
        print(predict)
    np.save('predict.npy', predict)
    f.close()
